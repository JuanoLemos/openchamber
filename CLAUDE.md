...
| `$MECANICAS` | `doc/mecanicas/` | Mecánicas y reglas de operación |
| `$TESTING` | *(definido en `$HARNESS`)* | `bun run type-check && bun run lint` |
...
|---|---|---|
| /CBP | Global | Commit + BUILD + Plan (orquestador vinculante) |
| /commit | Global | Commit rápido con formato Conventional Commits |
| /plan | Global | Planificar tarea en modo lectura |
| /debug | Global | Análisis profundo de sección |
| /health | Global | Verificar integridad del código |
| /doctor | Global | Cuidado integral del proyecto (3 fases) |
| /version | Global | Bump semver + CHANGELOG + PRE-FLIGHT |
| /updoc | Global | Actualizar documentación (sync + auditoría) |
| /explica | Global | Análisis de sección con scope automático |
| /limpiar | Global | Eliminar archivos temporales |
| /estado | Local | Reporte rápido del proyecto |
| /foco | Local | Cargar contexto de área (tx/ui/ux) |
| /backup | Global | Backup de archivos críticos |
| /bug | Global | Registrar bug en tracker |
| /incidente | Global | Registrar incidente runtime |
| /diligencia-check | Global | Validar estructura Diligencia |
| /salud | Global | Reporte de salud integral |
| /informe-salud | Global | Informe inter-proyecto |
| /legal | Global | Generar NOTICE + atribuciones |
| /rm | Local | Revisar Roadmap |
| /checklist | Local | Cruce RM + Checklist |

Comandos completos en `.opencode/commands/` (47 comandos: 44 globales + 3 locales del proyecto).
